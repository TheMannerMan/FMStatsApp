		...
		public List<Player> ParsedPlayers(Stream htmlFileStream)
		{
			...
			foreach (var row in rows)
			{
				...
				var player = new Player
				{
					...
					Acceleration = int.TryParse(columns[16].InnerText.Trim(), out var acc) ? acc : 0,
					...
					Kicking = int.TryParse(columns[36].InnerText.Trim(), out var kic) ? kic : 0,
					...
					Vision = int.TryParse(columns[52].InnerText.Trim(), out var vis) ? vis : 0,
